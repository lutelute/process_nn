...
| `viz/gnn.html` | **グラフNN（GNN）** — 「隣から集めて混ぜる」メッセージパッシングを可視化し、ラベル 2 個の半教師ありノード分類を GCN で実学習。鎖グラフで「層数＝届く距離」も実証 |
...
| `viz/surrogate.html` | **サロゲートモデル（代理モデル＋ベイズ最適化）** — 高価な関数を GP で「不確かさ付きの安い代理」に置き換え、獲得関数で次に測る 1 点を賢く選ぶ。グリッド探索との勝負つき |
| `viz/surrogate2.html` | **サロゲートモデル②（ブラックボックスの複製）** — 開けられない箱を入出力だけから小型 NN で写し取り（実学習）、速くて微分できる替え玉に。内挿は信用・外挿は創作、を実証 |
